...
describe Event do
  before :each do
        ...
        @user2.save
        @event = Event.new(date: (DateTime.now + rand(15)), description: "A test event ", location: "9 Back Hill, London, N1", name: "Event")
  ...
  describe "#initialize" do
    ...
    it "should hold the passed variables defining the event" do
      answer = 'A test event ', '9 Back Hill, London, N1', 'Event'
      expect([@event.description, @event.location, @event.name]).to eql answer
    end
  end
